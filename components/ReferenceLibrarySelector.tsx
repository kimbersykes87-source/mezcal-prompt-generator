...
export default function ReferenceLibrarySelector({ 
  ...
  useEffect(() => {
    async function loadCards() {
      try {
        ...
        setAllCards(cards);
        onReferencesLoad(cards.filter(card => 
          selection.faceCardIds.includes(card.id) || card.id === selection.backCardId
        ));
      } catch (error) {
  ...
  const getFullImageSrc = (card: CardReference): string => {
    if (imageSourceType === 'artwork') {
      const artworkPath = card.artworkPng || card.artworkPath;
      if (artworkPath) {
        return artworkPath;
      }
      // Fall back to card image if artwork doesn't exist (for face cards K, Q, J)
      return card.fullCardJpg || card.fullCardPath || '/card-images/057_card-back.jpg';
    }
    return card.fullCardJpg || card.fullCardPath || '/card-images/057_card-back.jpg';
  };
                    ...
                    <div className="p-2">
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        {filteredFaceCards.map(card => {
                          ...
                          const thumbnailSrc = getThumbnailSrc(card);
